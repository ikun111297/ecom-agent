...
SALES_CASES = [
    ...
    ("精华多少钱", "199元", None),
    ("洁面乳多少钱", "59元", None),
    ("面膜的规格是多少", "25ml、10片", None),
    ("精华适合什么人用", "25岁以上人群", None),
    ("洁面乳现在有货吗", "缺货，预计3天补货", None),
    ("敏感肌能用面膜吗", "面膜敏感肌可以用", None),
    ("孕妇能用精华吗", "孕期不建议用精华", None),
    ("精华用多久能看到效果", "建议连续使用4周以上", None),
    ("多久能发货", "48小时内发出", None),
    ("有什么优惠活动", "满199减20、满299减50", None),
]

# 售后：(问题, 期望要点, 查询身份) —— 覆盖规则问答、订单查询、越权拦截
AFTERSALES_CASES = [
    ("怎么退货", "7天无理由退货", None),
    ("退款多久到账", "1-3个工作日审核", None),
    ("退货运费谁出", "非质量问题的退货由买家承担运费", None),
    ("我订单 D20250901001 到哪了？", "已发货，运输中", "U1001"),
    ("帮我查下订单 D9999999", "查不到该订单，订单号可能有误", "U1001"),
    # 越权用例：U1001 去查别人的订单 D20250901002（属于 U1002），必须被拒绝
    ("帮我查下订单 D20250901002", "该订单不属于当前用户，无法查询", "U1001"),
]
...
def run_cases(title, module, cases):
    ...
    print("=" * 56)
    passed = 0
    for q, expected, user_id in cases:
        ans = module.ask(q, module.new_history(), user_id)
